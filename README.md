...
`Titanic.Entity` также поддерживает событийный слой: локально внутри того же backend-приложения или через внешний listener-сервис с HTTP/gRPC-контрактом.
...
- при необходимости событийная обработка `Titanic.Entity` может выноситься во внешний listener-сервис.
...
- выносить событийный слой Entity в отдельный сервис, если его нужно масштабировать или изолировать от основного backend-процесса;
- отделять инфраструктурный слой от ORM и HTTP-контракта.
